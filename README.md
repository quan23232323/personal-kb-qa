...
在 21 篇 / 699 个分块的演示库上，用 25 条**改写式**问题（正解 = 对应文档）对比三种策略：
...
**RRF 融合把首位命中从 52% 提到 68%（+16pp）、MRR 从 0.656 提到 0.753。**
语义路擅长「换句话问」，关键词路擅长术语与编号，两路是互补而非冗余关系。
代价是 R@5 由 92% 略降到 84%——融合会把「只有一路强命中」的候选挤出前 5，
而下游还有相似度阈值与关键词分下限二次过滤兜底。

评测设置、逐条明细与已知局限见 [`docs/retrieval-eval.md`](./docs/retrieval-eval.md)。
复现：`cd backend && PKB_CONFIG=config.demo.yaml python retrieval_eval.py`
